...
const CHROME = process.env.NOCMS_CHROME
...
const normalise = (html) => html
  ...
  .replace(/<\/div>/g, (m, i, s) => m)          // wrappers are removed above by pairing
  .replace(/\s+/g, ' ')
  .replace(/>\s+</g, '><')
  .trim();
...
function unwrap(html) {
  ...
  while (i < html.length) {
    ...
    if (!m || m.index !== i) { out += html[i]; i++; continue; }
...
try {
  ...
  for (const target of pages) {
    ...
    const editorHtml = unwrap(bodyOf(await page.evaluate(() => window.__nocms.renderTree())));
    const built = fs.readFileSync(path.resolve(target.dist), 'utf-8');
    const body = bodyOf(built);

    if (normalise(editorHtml) !== normalise(body)) {
      const a = normalise(editorHtml), b = normalise(body);
      let at = 0; while (at < Math.max(a.length, b.length) && a[at] === b[at]) at++;
      failures.push([
        `  ${target.route}`,
