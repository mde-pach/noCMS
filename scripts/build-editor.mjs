...
await build({
  ...
  resolve: {
    alias: [
      ...
      { find: /^node:async_hooks$/, replacement: fileURLToPath(new URL('./async-hooks-shim.mjs', import.meta.url)) },
    ],
  },
...
await copyFile(fileURLToPath(new URL('../.nocms-build/editor.js', import.meta.url)), out + 'editor.js');
await copyFile(
  fileURLToPath(new URL('../node_modules/@astrojs/compiler/dist/astro.wasm', import.meta.url)),
  out + 'astro.wasm',
);
await rm(fileURLToPath(new URL('../.nocms-build', import.meta.url)), { recursive: true, force: true });
