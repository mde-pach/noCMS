  ...
  return {
    ...
    async read(path) {
      try {
        ...
        return new TextDecoder().decode(Uint8Array.from(atob(r.content), (c) => c.charCodeAt(0)));
      } catch { return null; }
    },
    async list(glob) {
    ...
    describeTarget() { return `${owner}/${repo}`; },
  };
}
