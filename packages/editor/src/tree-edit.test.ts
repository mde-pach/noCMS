...
import { buildJsxElement } from "./jsx-attributes.js";
...
const leaf = (name: string): Nodes => buildJsxElement(name) as Nodes;
// A container with a text-bearing child, so serialize→re-parse is a fair round-trip
// (an empty paragraph collapses to a self-closing tag and wouldn't exercise the slot).
const container = (name: string): Nodes =>
  ({
...
describe("tree-edit transforms (D15)", () => {
  ...
  test("insertAt clamps an out-of-range index to the end", () => {
    const doc = parseMdx(SRC);
    const next = insertAt(doc, [], 99, leaf("Image"));
  ...
  test("an unresolvable address returns the tree unchanged", () => {
    ...
    expect(insertAt(doc, [99], 0, leaf("Image"))).toBe(doc);
  });
});
...
describe("canonical MDX round-trip after a transform (D15)", () => {
  ...
  const transforms: Record<string, (doc: MdxDocument) => MdxDocument> = {
    ...
    insertContainer: (doc) => insertAt(doc, [], 0, containerNode),
