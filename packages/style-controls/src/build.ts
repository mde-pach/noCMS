...
import type { Catalog, ColorFamily, Control, Feature, FeatureOption } from "./types";
...
/**
 * Build the design-control catalog from a flat token file by loading Tailwind v4's design system
 * and reshaping every utility into a CSS feature. `base` is the directory the engine resolves
 * `@import "tailwindcss"` against (the consuming site's root). Node-only — pulls in the v4 engine.
 */
export async function buildCatalog(tokensText: string, base: string): Promise<Catalog> {
