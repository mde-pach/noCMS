...
function resolveSafe(rel, forWrite) {
  ...
  if (forWrite && !WRITABLE.some((d) => relative === d || relative.startsWith(d + path.sep))) {
...
test('refuses writes outside editor-owned directories', () => {
  for (const bad of ['package.json', 'src/lib/page-tree.mjs', 'node_modules/x/index.js', '.github/workflows/deploy.yml']) {
    assert.throws(() => resolveSafe(bad, true), /not writable/);
  }
});

test('allows writes the editor legitimately makes', () => {
  for (const ok of ['src/pages/index.astro', 'src/pages/about/index.astro', 'src/styles/theme.css', 'public/media/a.webp']) {
    assert.equal(resolveSafe(ok, true), path.join(ROOT, ok));
  }
});

test('reads are allowed more broadly than writes', () => {
  assert.equal(resolveSafe('src/sections/hero/index.astro', false), '/project/src/sections/hero/index.astro');
