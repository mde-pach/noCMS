...
import { parsePage, serializePage, literalValue, sections } from '../src/lib/page-tree.mjs';
...
test('sections() finds components at any depth, not html', async () => {
  const page = await parsePage(`<div><Hero title="a" /><Columns><Hero title="b" /></Columns></div>\n`);
  assert.deepEqual(sections(page.body).map((s) => s.node.name), ['Hero', 'Columns', 'Hero']);
});
...
test('imports bind tags to modules, and aliases are followed', async () => {
  ...
  assert.deepEqual(Object.keys(parseImports(page.frontmatter)), ['FeatureGrid', 'Whatever']);
});
...
test('ensureImport adds a missing binding and never duplicates one', async () => {
  ...
  assert.equal(ensureImport(page, 'CallToAction', '../sections/call-to-action/index.astro'), true);
  assert.equal(ensureImport(page, 'CallToAction', '../sections/call-to-action/index.astro'), false);

  const out = serializePage(page);
  assert.equal((out.match(/import CallToAction/g) || []).length, 1);
  assert.ok(out.includes("import Hero from '../sections/hero/index.astro';"), 'existing import kept');
